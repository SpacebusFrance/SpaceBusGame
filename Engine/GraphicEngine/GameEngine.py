...
from Engine.ControlScreens.MainScreen import ControlScreen
...
from Engine.Utils.Sounds import SoundManager
from Engine.Utils.utils import read_ini_file, smart_cast, Logger
...
class Game(ShowBase):
    ...
    def generate_spacebus_screens(self):
        ...
        if self.params("game_mode").lower() == 'debug':
            ...
            self.control_screen.set_single_window(screen_numbers=2, res_x=800, res_y=600, decorated=True, pos=(200, 200))
            self.screens.append(FakeScreen3D(self, 1))

        elif self.params("game_mode").lower() == 'only_control_screen':
            self.control_screen.set_single_window(screen_numbers=1, res_x=800, res_y=600, decorated=True, pos=(200, 200))
    ...
    def update_soft_state(self, state_key, value, force_power=False, silent=False):
        ...
        if state_key in self.soft_states:
            if (not self.params("use_power") or force_power or not value or (
                ...
                # now we switch it on
                Logger.print("(soft_state): ", state_key, ':', value, color='yellow')
                ...
                if state_key == "pilote_automatique_failed" and value:
                    ...
                    Logger.print("(soft_state): ", state_key, ':', False, color='yellow')
            else:
                if state_key.startswith('moteur') and value:
                    ...
                    if state_key == "pilote_automatique":
                        ...
                        Logger.print("(soft_state): ", state_key, ':', True, color='yellow')
                        self.scenario.update_shuttle_state()
                        self.soft_states["pilote_automatique_failed"] = False
                        Logger.print("(soft_state): ", state_key, ':', False, color='yellow')
            return True
    ...
    def update_hard_state(self, state_key, value, silent=False):
        ...
        # only do it if the key exists and if we listen to hardware and if the value is different from the previous one (to avoid spamminf of joysticks_axis = 0.0
            ...
            state_key]:
            Logger.print("(hard_state): ", state_key, ':', value, color='yellow')
            self.hard_states[state_key] = value
